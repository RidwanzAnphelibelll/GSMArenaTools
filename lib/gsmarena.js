...
const isMatchingPhone = (phoneName, searchQuery) => {
    const normalizedPhone = normalizeQuery(phoneName);
...
const searchPhones = async (searchQuery) => {
  try {
    ...
    $('.general-menu ul li').each((i, el) => {
      ...
      if (href && name && isMatchingPhone(name, searchQuery)) {
        phones.push({
          name: name,
          url: href,
          image: img
    ...
    $decrypted('a').each((i, el) => {
      ...
      if (href && href.includes('.php') && name && isMatchingPhone(name, searchQuery)) {
        modelPhones.push({
          name: name,
          url: href,
          image: img || ''
...
const getFirstUrl = async (searchQuery) => {
  try {
    ...
    let $ = cheerio.load(quickResponse.data);
    const quickFirstLinkElement = $('.general-menu ul li').first();
    const quickFirstName = quickFirstLinkElement.find('strong').text().trim();
    const quickFirstLink = quickFirstLinkElement.find('a').attr('href');
    
    if (quickFirstLink && isMatchingPhone(quickFirstName, searchQuery)) {
    ...
    let firstMatchingUrl = null;
    $decrypted('a').each((i, el) => {
      const href = $decrypted(el).attr('href');
      const name = $decrypted(el).text().trim();
      
      if (href && href.includes('.php') && name && isMatchingPhone(name, searchQuery)) {
        firstMatchingUrl = href;
        return false;
      }
    });
