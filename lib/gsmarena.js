...
const searchPhones = async (searchQuery) => {
  try {
    ...
    $('.makers ul li').each((i, el) => {
      ...
      const name = $link.find('span').html();
      const img = $link.find('img').attr('src');
      
      if (href && name && href.includes('.php')) {
        phones.push({
          name: name.replace(/<br\s*\/?>/gi, ' ').replace(/<[^>]*>/g, '').trim(),
          url: href,
          image: img
        });
      }
...
const getFirstUrl = async (searchQuery) => {
  try {
    ...
    let $ = cheerio.load(quickResponse.data);
    const quickFirstLink = $('.makers ul li').first().find('a').attr('href');
    
    if (quickFirstLink && quickFirstLink.includes('.php')) {
